...
<main class="professional-home">
  <div class="ph-hero-wrap"><section class="ph-hero<?= $hero ? '' : ' ph-hero-fallback' ?>"><?php if($hero): ?><img src="<?= e($hero) ?>" alt="<?= e($name) ?>" fetchpriority="high"><?php endif; ?><div class="ph-overlay"></div><div class="ph-hero-copy"><span class="ph-eyebrow"><?= e($copy['hero_eyebrow']) ?><?= $company?->getAttribute('city')?' · '.e($company->getAttribute('city')):'' ?></span><h1><?= e($page->getAttribute('h1')?:$copy['hero_title']) ?></h1><?php if($company?->getAttribute('short_description')): ?><p><?= e($company->getAttribute('short_description')) ?></p><?php endif; ?><div><a class="ph-btn yellow" href="#devis">Demander un devis</a><?php if($phone!==''): ?><a class="ph-btn white" href="tel:<?= e($phoneHref) ?>">Appeler — <?= e($phone) ?></a><?php endif; ?></div></div></section></div>

  <?php if($services): ?><section class="ph-section" id="services"><span class="ph-eyebrow blue">Nos prestations</span><h2><?= e($copy['services_title']) ?></h2><div class="marketing-service-grid"><?php foreach($services as $service):$image=$mediaUrl($service->getAttribute('image_media_id')); ?><a class="marketing-service-card" href="/<?= e($service->getAttribute('slug')) ?>"><?php if($image): ?><img src="<?= e($image) ?>" alt="<?= e($service->getAttribute('public_name')) ?>" loading="lazy"><?php else: ?><div class="marketing-image-empty" aria-hidden="true"><?= e(mb_strtoupper(mb_substr((string)$service->getAttribute('public_name'),0,1))) ?></div><?php endif; ?><div><h3><?= e($service->getAttribute('public_name')) ?></h3><p><?= e($service->getAttribute('description')) ?></p><span>Découvrir →</span></div></a><?php endforeach; ?></div></section><?php endif; ?>
