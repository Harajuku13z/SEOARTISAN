...
if (!$relatedServices) {
    ...
    foreach (CompanyService::all('sort_order ASC') as $candidate) if ($candidate->getAttribute('is_active') && $candidate->id() !== $service?->id() && ($keyword === '' || str_contains((string)$candidate->getAttribute('slug'),$keyword))) $relatedServices[]=$candidate;
}
...
$heroImage=trim((string)($savedCopy['hero_image']??''));$heroImage=preg_match('#^/(assets|uploads)/[A-Za-z0-9/_.-]+$#',$heroImage)?$heroImage:$image;
$localCities=[];
$localLinks=[];foreach(CompanyLocation::where(['is_active'=>1]) as $location){$localCity=City::find((int)$location->getAttribute('city_id'));if(!$localCity)continue;$lp=$service?\App\Models\LocalPage::first(['company_service_id'=>(int)$service->id(),'city_id'=>(int)$localCity->id(),'is_active'=>1]):null;$lpPage=$lp?\App\Models\Page::find((int)$lp->getAttribute('page_id')):null;$localLinks[(string)$localCity->getAttribute('name')]=$lpPage&&$lpPage->getAttribute('status')==='published'?'/'.$lpPage->getAttribute('slug'):null;}$localCities=array_keys($localLinks);if(!array_filter($localLinks))$localCities=array_slice($localCities,0,10);
