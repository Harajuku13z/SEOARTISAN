...
final class SmtpMailer
{
    ...
    {
        $cfg=MailSettings::resolved();$host=$cfg['host'];$port=$cfg['port'];$username=$cfg['username'];$password=$cfg['password'];$from=$cfg['from_address']?:$username;$fromName=$cfg['from_name'];$replyTo=$replyTo?:$cfg['reply_to'];$secure=$cfg['encryption']==='ssl'||$port===465;$hello=parse_url((string)config('app.url',''),PHP_URL_HOST)?:'localhost';
        if($username===''||$password===''||!filter_var($to,FILTER_VALIDATE_EMAIL))throw new RuntimeException('Configuration SMTP incomplète.');
        $socket=@stream_socket_client(($secure?"ssl":"tcp")."://{$host}:{$port}",$number,$message,15);if(!is_resource($socket))throw new RuntimeException("Connexion SMTP impossible : {$message} ({$number}).");stream_set_timeout($socket,15);
        try{$this->expect($socket,[220]);$this->command($socket,'EHLO '.$hello,[250]);if(!$secure){$this->command($socket,'STARTTLS',[220]);if(!stream_socket_enable_crypto($socket,true,STREAM_CRYPTO_METHOD_TLS_CLIENT))throw new RuntimeException('Activation TLS SMTP impossible.');$this->command($socket,'EHLO '.$hello,[250]);}$this->command($socket,'AUTH LOGIN',[334]);$this->command($socket,base64_encode($username),[334]);$this->command($socket,base64_encode($password),[235]);$this->command($socket,'MAIL FROM:<'.$from.'>',[250]);$this->command($socket,'RCPT TO:<'.$to.'>',[250,251]);$this->command($socket,'DATA',[354]);
