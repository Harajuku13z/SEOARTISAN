...
final class OpenAIProvider implements AIProviderInterface
{
    ...
    public function generate(string $systemPrompt, string $userPrompt, float $temperature, int $maxTokens): AIGenerationResult
    {
        ...
        if ($status < 200 || $status >= 300 || !is_array($decoded)) {
            return AIGenerationResult::fail("OpenAI a repondu avec le statut {$status} : " . substr((string) $body, 0, 300));
        }

        $content = $decoded['choices'][0]['message']['content'] ?? null;
        if (!is_string($content)) {
            return AIGenerationResult::fail('Reponse OpenAI sans contenu exploitable.');
        }
    ...
    public function testConnection(): array
    {
        ...
        return ['ok' => true, 'message' => 'Connexion OpenAI reussie.'];
    }
