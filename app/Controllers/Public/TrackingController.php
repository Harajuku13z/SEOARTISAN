...
final class TrackingController
{
    ...
    public function event(Request $request): Response
    {
        ...
        ConversionEvent::create(['event_type'=>$type,'element_text'=>mb_substr(trim((string)$request->input('element_text','')),0,255)?:null,'target_url'=>mb_substr(trim((string)$request->input('target_url','')),0,500)?:null,'page_path'=>mb_substr(trim((string)$request->input('page_path','/')),0,500),'location_label'=>$location?:null,'session_id'=>mb_substr(trim((string)$request->input('session_id','')),0,64)?:null,'ip_hash'=>hash('sha256',$request->ip().'|'.csrf_token()),'ip_encrypted'=>$this->encryptIp($request->ip()),'user_agent'=>mb_substr($request->userAgent(),0,255),'is_human'=>$assessment['is_human'],'bot_score'=>$assessment['score'],'rejection_reason'=>$assessment['reasons']?implode(',',$assessment['reasons']):null,'metadata'=>['interactions'=>(int)$request->input('_human_interactions',0),'elapsed'=>(int)$request->input('_human_elapsed',0)]]);
    ...
    private function encryptIp(string $ip): ?string
    {
        ...
        try { return Crypto::encrypt($ip, (string) config('app.key', '')); } catch (\Throwable) { return null; }
    }
}
